...
// http://svay.com/blog/index/post/2009/06/19/Face-detection-in-pure-PHP-%28without-OpenCV%29
/*
$detector = new SvayFaceDetector('SvayFaceDetection.dat');
$detector->face_detect('maurice_svay_150.jpg');
$detector->setBorderColor(128, 66, 44);
$detector->toJpeg();
*/
class SvayFaceDetector {
    ...
    public function __construct($detection_file = 'SvayFaceDetection.dat') {
	...
	public function setBorderColor($r, $g, $b) {
		if ($this->canvas) {
			$this->border_color = imagecolorallocate($this->canvas, $r, $g, $b);
		}
	}
 
    public function toJpeg() {
		if (!isset($this->border_color)) {
			$this->border_color = imagecolorallocate($this->canvas, 255, 0, 0); //red
		}
        imagerectangle($this->canvas, $this->face['x'], $this->face['y'], $this->face['x']+$this->face['w'], $this->face['y']+ $this->face['w'], $this->border_color);
